...
public interface FileInfoModelRepository extends JpaRepository<FileInfoModel, Integer> {
    ...
    @Query("SELECT n FROM FileInfoModel n WHERE n.exchangeCode= :exchangeCode AND n.isSettlement = :isSettlement AND n.uploadDateTime BETWEEN :startDate AND :endDate")
    List<FileInfoModel> getSettlementDataByExchangeCode(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate, @Param("exchangeCode") String exchnageCode, @Param("isSettlement") int isSettlement);
