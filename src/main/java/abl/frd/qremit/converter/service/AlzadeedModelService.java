...
import abl.frd.qremit.converter.repository.AlzadeedModelRepository;
...
@Service
public class AlzadeedModelService {
    @Autowired
    AlzadeedModelRepository alzadeedModelRepository;
    @Autowired
    ...
    public Map<String, Object> save(MultipartFile file, int userId, String exchangeCode, String nrtaCode, String tbl) {
        ...
        try
        {
            ...
            Map<String, Object> alzadeedData = csvToAlzadeedModels(file.getInputStream(), user, fileInfoModel, exchangeCode, nrtaCode, currentDateTime, tbl);
            ...
            if(alzadeedData.containsKey("errorMessage")){
                resp.put("errorMessage", alzadeedData.get("errorMessage"));
            }
            if(alzadeedData.containsKey("errorCount") && ((Integer) alzadeedData.get("errorCount") >= 1)){
                int errorCount = (Integer) alzadeedData.get("errorCount");
                fileInfoModel.setErrorCount(errorCount);
                resp.put("fileInfoModel", fileInfoModel);
                fileInfoModelRepository.save(fileInfoModel);
            }

            if(alzadeedModels.size()!=0) {
                for(AlzadeedModel alzadeedModel : alzadeedModels){
                    alzadeedModel.setFileInfoModel(fileInfoModel);
                    alzadeedModel.setUserModel(user);
                }
                // 4 DIFFERENTS DATA TABLE GENERATION GOING ON HERE
                Map<String, Object> convertedDataModels = commonService.generateFourConvertedDataModel(alzadeedModels, fileInfoModel, user, currentDateTime, 0);
                fileInfoModel = CommonService.countFourConvertedDataModel(convertedDataModels);
                fileInfoModel.setTotalCount(String.valueOf(alzadeedModels.size()));
                fileInfoModel.setIsSettlement(0);
                fileInfoModel.setAlzadeedModel(alzadeedModels);
    ...
    public Map<String, Object> csvToAlzadeedModels(InputStream is, User user, FileInfoModel fileInfoModel, String exchangeCode, String nrtaCode, LocalDateTime currentDateTime, String tbl) {
        Map<String, Object> resp = new HashMap<>();
        Optional<AlzadeedModel> duplicateData = Optional.empty();
        try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
            ...
            modelResp = commonService.processDataToModel(dataList, fileInfoModel, user, uniqueDataList, archiveDataList, currentDateTime, duplicateData, AlzadeedModel.class, resp, errorDataModelList, fileExchangeCode, 0, 0);
            List<AlzadeedModel> alzadeedDataModelList = (List<AlzadeedModel>) modelResp.get("modelList");
            ...
            if(errorDataModelList.isEmpty() && alzadeedDataModelList.isEmpty()){
