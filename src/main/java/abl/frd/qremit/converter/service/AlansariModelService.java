...
import abl.frd.qremit.converter.repository.AlansariModelRepository;
...
public class AlansariModelService {
    ...
    public Map<String, Object> save(MultipartFile file, int userId, String exchangeCode, String nrtaCode, String tbl) {
        ...
        try
        {
            ...
            Map<String, Object> alansariData = csvToAlansariModels(file.getInputStream(), user, fileInfoModel, exchangeCode, nrtaCode, currentDateTime, tbl);
    ...
    public Map<String, Object> csvToAlansariModels(InputStream is, User user, FileInfoModel fileInfoModel, String exchangeCode, String nrtaCode, LocalDateTime currentDateTime, String tbl) {
        Map<String, Object> resp = new HashMap<>();
        Optional<AlansariModel> duplicateData = Optional.empty();
        try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(is, "UTF-8"))){
            ...
            try(CSVParser csvParser = new CSVParser(new StringReader(fileContentWithUniqueHeader),CSVFormat.newFormat('|').withFirstRecordAsHeader().withIgnoreHeaderCase().withTrim())){
                ...
                modelResp = CommonService.processDataToModel(dataList, fileInfoModel, user, uniqueDataList, archiveDataList, currentDateTime, duplicateData, AlansariModel.class, resp, errorDataModelList, fileExchangeCode, 0, 0);
                List<AlansariModel> alansariDataModelList = (List<AlansariModel>) modelResp.get("modelList");
