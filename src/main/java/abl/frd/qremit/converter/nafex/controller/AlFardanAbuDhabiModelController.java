...
import abl.frd.qremit.converter.nafex.service.AlFardanAbuDhabiModelService;
import abl.frd.qremit.converter.nafex.service.CommonService;
import abl.frd.qremit.converter.nafex.service.MyUserDetailsService;
import java.util.*;
@Controller
public class AlFardanAbuDhabiModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final CommonService commonService;
    @Autowired
    AlFardanAbuDhabiModelService alFardanAbuDhabiModelService;
    @Autowired
    public AlFardanAbuDhabiModelController(MyUserDetailsService myUserDetailsService, CommonService commonService) {
        this.myUserDetailsService = myUserDetailsService;
        this.commonService = commonService;
    }
    @PostMapping("/alfardan_abudhabiUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, 
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())){
                try {
                    Map<String, Object> resp = alFardanAbuDhabiModelService.save(file, userId, exchangeCode, nrtaCode);
