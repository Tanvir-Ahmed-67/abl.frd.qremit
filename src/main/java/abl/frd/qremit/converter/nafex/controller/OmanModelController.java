...
import abl.frd.qremit.converter.nafex.service.OmanModelService;
import java.util.*;

@Controller
public class OmanModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final OmanModelService omanModelService;
    private final CommonService commonService;
    
    @Autowired
    public OmanModelController(OmanModelService omanModelService,MyUserDetailsService myUserDetailsService, CommonService commonService){
        this.myUserDetailsService = myUserDetailsService;
        this.omanModelService = omanModelService;
        this.commonService = commonService;
    }
    
    @PostMapping("/omanUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, @ModelAttribute("exchangeCode") String exchangeCode,
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())) {
                try {
                    Map<String, Object> resp = omanModelService.save(file, userId, exchangeCode, nrtaCode);
