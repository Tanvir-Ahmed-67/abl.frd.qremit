...
import abl.frd.qremit.converter.nafex.service.SwiftModelService;
import java.util.*;

@Controller
public class SwiftModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final SwiftModelService swiftModelService;
    private final CommonService commonService;
    
    @Autowired
    public SwiftModelController(SwiftModelService swiftModelService,MyUserDetailsService myUserDetailsService, CommonService commonService){
        this.myUserDetailsService = myUserDetailsService;
        this.swiftModelService = swiftModelService;
        this.commonService = commonService;
    }
    
    @PostMapping("/swiftUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, @ModelAttribute("exchangeCode") String exchangeCode,
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())) {
                try {
                    Map<String, Object> resp = swiftModelService.save(file, userId, exchangeCode, nrtaCode);
