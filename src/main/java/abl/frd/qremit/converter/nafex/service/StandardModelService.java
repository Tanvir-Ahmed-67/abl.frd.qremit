...
import abl.frd.qremit.converter.nafex.repository.StandardModelRepository;
...
public class StandardModelService {
    @Autowired
    StandardModelRepository standardModelRepository;
    @Autowired
    ...
    public Map<String, Object> save(MultipartFile file, int userId, String exchangeCode, String nrtaCode) {
        ...
        try
        {
            ...
            Map<String, Object> standardData = csvToStandardModels(file.getInputStream(), user, fileInfoModel, exchangeCode, nrtaCode, currentDateTime);
            List<StandardModel> standardModels = (List<StandardModel>) standardData.get("standardDataModelList");

            if(standardData.containsKey("errorMessage")){
                resp.put("errorMessage", standardData.get("errorMessage"));
            }
            if(standardData.containsKey("errorCount") && ((Integer) standardData.get("errorCount") >= 1)){
                int errorCount = (Integer) standardData.get("errorCount");
            ...
            if(standardModels.size()!=0) {
                for(StandardModel standardModel : standardModels){
                    standardModel.setFileInfoModel(fileInfoModel);
                    standardModel.setUserModel(user);
                }
                // 4 DIFFERENTS DATA TABLE GENERATION GOING ON HERE
                Map<String, Object> convertedDataModels = CommonService.generateFourConvertedDataModel(standardModels, fileInfoModel, user, currentDateTime, 0);
                fileInfoModel = CommonService.countFourConvertedDataModel(convertedDataModels);
                fileInfoModel.setTotalCount(String.valueOf(standardModels.size()));
                fileInfoModel.setIsSettlement(0);
                fileInfoModel.setStandardModel(standardModels);
    ...
    public Map<String, Object> csvToStandardModels(InputStream is, User user, FileInfoModel fileInfoModel, String exchangeCode, String nrtaCode, LocalDateTime currentDateTime) {
        Map<String, Object> resp = new HashMap<>();
        ...
        try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(is, "UTF-16LE"));
            ...
            for (CSVRecord csvRecord : csvRecords) {
                ...
                duplicateData = standardModelRepository.findByTransactionNoIgnoreCaseAndAmountAndExchangeCode(transactionNo, CommonService.convertStringToDouble(amount), exchangeCode);
                String beneficiaryAccount = "COC" + csvRecord.get(7).trim();
                ...
                StandardModel standardDataModel = new StandardModel();
                standardDataModel = CommonService.createDataModel(standardDataModel, data);
                standardDataModel.setTypeFlag(CommonService.setTypeFlag(beneficiaryAccount, bankName, branchCode));
                standardDataModel.setUploadDateTime(currentDateTime);
                standardDataModelList.add(standardDataModel);
            }
            ...
            if(errorDataModelList.isEmpty() && standardDataModelList.isEmpty()){
                fileInfoModelService.deleteFileInfoModelById(fileInfoModel.getId());
            }
            resp.put("standardDataModelList", standardDataModelList);
