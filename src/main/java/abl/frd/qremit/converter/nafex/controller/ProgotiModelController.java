...
import abl.frd.qremit.converter.nafex.service.ProgotiModelService;
...
@Controller
public class ProgotiModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final CommonService commonService;
    @Autowired
    ProgotiModelService progotiModelService;

    @Autowired
    public ProgotiModelController(MyUserDetailsService myUserDetailsService, CommonService commonService){
        this.myUserDetailsService = myUserDetailsService;
        this.commonService = commonService;
    }

    @PostMapping("/progotiUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, 
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())){
                try {
                    Map<String, Object> resp = progotiModelService.save(file, userId, exchangeCode, nrtaCode);
