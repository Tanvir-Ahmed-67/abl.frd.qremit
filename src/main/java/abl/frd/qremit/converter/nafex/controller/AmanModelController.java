...
import abl.frd.qremit.converter.nafex.service.AmanModelService;
import java.util.*;

@Controller
public class AmanModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final AmanModelService amanModelService;
    private final CommonService commonService;
    
    @Autowired
    public AmanModelController(AmanModelService amanModelService,MyUserDetailsService myUserDetailsService, CommonService commonService){
        this.myUserDetailsService = myUserDetailsService;
        this.amanModelService = amanModelService;
        this.commonService = commonService;
    }
    
    @PostMapping("/amanUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, @ModelAttribute("exchangeCode") String exchangeCode,
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())) {
                try {
                    Map<String, Object> resp = amanModelService.save(file, userId, exchangeCode, nrtaCode);
