...
import abl.frd.qremit.converter.nafex.service.SunmanModelService;
import java.util.*;

@Controller
public class SunmanModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final SunmanModelService sunmanModelService;
    private final CommonService commonService;
    
    @Autowired
    public SunmanModelController(SunmanModelService sunmanModelService,MyUserDetailsService myUserDetailsService, CommonService commonService){
        this.myUserDetailsService = myUserDetailsService;
        this.sunmanModelService = sunmanModelService;
        this.commonService = commonService;
    }
    
    @PostMapping("/sunmanUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, @ModelAttribute("exchangeCode") String exchangeCode,
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())) {
                try {
                    Map<String, Object> resp = sunmanModelService.save(file, userId, exchangeCode, nrtaCode);
