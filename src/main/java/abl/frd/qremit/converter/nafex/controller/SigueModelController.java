...
import abl.frd.qremit.converter.nafex.service.SigueModelService;
import java.util.*;

@Controller
public class SigueModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final SigueModelService sigueModelService;
    private final CommonService commonService;
    
    @Autowired
    public SigueModelController(SigueModelService sigueModelService,MyUserDetailsService myUserDetailsService, CommonService commonService){
        this.myUserDetailsService = myUserDetailsService;
        this.sigueModelService = sigueModelService;
        this.commonService = commonService;
    }
    
    @PostMapping("/sigueUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, @ModelAttribute("exchangeCode") String exchangeCode,
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())) {
                try {
                    Map<String, Object> resp = sigueModelService.save(file, userId, exchangeCode, nrtaCode);
