...
import abl.frd.qremit.converter.nafex.service.NblUsaModelService;
...
@Controller
public class NblUsaModelController {
    private final NblUsaModelService nblUsaModelService;
    private final MyUserDetailsService myUserDetailsService;
    private final CommonService commonService;

    @Autowired
    public NblUsaModelController(NblUsaModelService nblUsaModelService, MyUserDetailsService myUserDetailsService, CommonService commonService){
        this.nblUsaModelService = nblUsaModelService;
        this.myUserDetailsService = myUserDetailsService;
        this.commonService = commonService;
    }

    @PostMapping("/nblusaUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, 
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())){
                try {
                    Map<String, Object> resp = nblUsaModelService.save(file, userId, exchangeCode, nrtaCode);
