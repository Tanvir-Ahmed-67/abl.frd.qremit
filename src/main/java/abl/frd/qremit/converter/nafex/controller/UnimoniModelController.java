...
import abl.frd.qremit.converter.nafex.service.UnimoniModelService;
import java.util.*;

@Controller
public class UnimoniModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final UnimoniModelService unimoniModelService;
    private final CommonService commonService;
    
    @Autowired
    public UnimoniModelController(UnimoniModelService unimoniModelService,MyUserDetailsService myUserDetailsService, CommonService commonService){
        this.myUserDetailsService = myUserDetailsService;
        this.unimoniModelService = unimoniModelService;
        this.commonService = commonService;
    }
    
    @PostMapping("/unimoniUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, @ModelAttribute("exchangeCode") String exchangeCode,
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())) {
                try {
                    Map<String, Object> resp = unimoniModelService.save(file, userId, exchangeCode, nrtaCode);
