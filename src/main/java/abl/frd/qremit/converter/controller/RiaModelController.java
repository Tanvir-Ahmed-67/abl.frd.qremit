...
import abl.frd.qremit.converter.service.RiaModelService;
import java.util.*;
...
@Controller
public class RiaModelController {
    private final MyUserDetailsService myUserDetailsService;
    private final RiaModelService riaModelService;
    private final CommonService commonService;
    public RiaModelController(MyUserDetailsService myUserDetailsService, RiaModelService riaModelService, CommonService commonService){
        this.myUserDetailsService = myUserDetailsService;
        this.riaModelService = riaModelService;
        this.commonService = commonService;
    }
    @PostMapping("/riaUpload")
    public String uploadFile(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, @ModelAttribute("fileType") String fileType,
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())){
                try {
                    Map<String, Object> resp = riaModelService.save(file, userId, exchangeCode, fileType, nrtaCode, tbl);
