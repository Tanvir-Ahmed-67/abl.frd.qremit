...
import abl.frd.qremit.converter.service.NpsbMfsService;
import java.util.*;

@Controller
public class NpsbMfsController {
    @Autowired
    ...
    public String saveData(@AuthenticationPrincipal MyUserDetails userDetails, @ModelAttribute("file") MultipartFile file, @ModelAttribute("exchangeCode") String exchangeCode,
        ...
        if (CommonService.hasCSVFormat(file)) {
            if(!commonService.ifFileExist(file.getOriginalFilename())){
                try {
                    Map<String, Object> resp = npsbMfsService.save(file, userId, exchangeCode, tbl);
